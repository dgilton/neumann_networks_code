...
def sample_gramian(input):
    downsampled = tf.image.resize_images(input, size=[int(dimension1*factor), int(dimension2*factor)], method=tf.image.ResizeMethod.BICUBIC, align_corners=True)
    upsampled = tf.image.resize_images(downsampled, size=[dimension1, dimension2], method=tf.image.ResizeMethod.BICUBIC, align_corners=True)
    return upsampled

def sample_forward(input):
    downsampled = tf.image.resize_images(input, size=[int(dimension1*factor), int(dimension2*factor)], method=tf.image.ResizeMethod.BICUBIC, align_corners=True)
    return downsampled # + tf.random_normal(shape=tf.shape(downsampled), mean=0.0, stddev=std, dtype=tf.float32)

def sample_adjoint(input):
    upsampled = tf.image.resize_images(input, size=[dimension1, dimension2], method=tf.image.ResizeMethod.BICUBIC, align_corners=True)
    return upsampled
